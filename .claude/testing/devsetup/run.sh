#!/usr/bin/env bash
# .claude/testing/devsetup/run.sh — entry point for the install-dev-tools.sh test harness.
# Must run INSIDE the driver container (images/driver.Dockerfile) with /var/run/docker.sock
# mounted, mirroring .claude/testing/own-script/run.sh.
set -euo pipefail
IFS=$'\n\t'

: "${HOST_REPO_PATH:?HOST_REPO_PATH is not set}"

readonly TESTING_DIR="/work/repo/.claude/testing/devsetup"
...
main() {
  sep
  info "test_devsetup: starting run, results -> ${RESULTS_DIR}"
  sep
  ...
  CREATED_VOLUME="devsetup-test-aptcache-$$"
