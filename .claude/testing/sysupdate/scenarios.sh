...
run_all_scenarios() {
  sep; info "01 — fresh full run (apt update/full-upgrade/autoremove/autoclean, snap+flatpak absent)"; sep
  run_simple 01_FRESH_FULL_RUN 0 verify_fresh_run - run_update_system_all || true

  sep; info "02 — reboot-required file present"; sep
  run_simple 02_REBOOT_REQUIRED 0 verify_reboot_required presetup_reboot_required run_update_system_all || true

  sep; info "03 — non-root with NOPASSWD sudo"; sep
  run_simple 03_NONROOT_SUDO 0 verify_nonroot_sudo_run presetup_nonroot_user run_update_system_all -u updatetester || true

  sep; info "04 — non-root, sudo absent"; sep
  run_nonroot_no_sudo || true

  sep; info "05 — idempotency: re-run on the same container"; sep
  run_idempotent_rerun || true

  sep; info "install_sysupdate.sh: fresh install + idempotent reinstall"; sep
  run_install_fresh_then_idempotent || true

  sep; info "06 — install_sysupdate.sh: missing required command (wget)"; sep
  run_install_missing_wget || true

  sep; info "07 — install_sysupdate.sh: non-root with NOPASSWD sudo"; sep
  run_install_nonroot_sudo || true

  sep; info "08 — install_sysupdate.sh: non-root, sudo absent"; sep
  run_install_nonroot_no_sudo || true
}

# run_simple <id> <expected_exit> <verify_fn|-> <presetup_fn|-> <runner_fn> [runner_extra_args...]
run_simple() {
  local scen_id="$1" expected_exit="$2" verify_fn="$3" presetup_fn="$4" runner_fn="$5"; shift 5
  ...
  if [[ "$rc" != "$expected_exit" ]]; then
    status="FAIL"; note="exit=$rc, expected $expected_exit (see $log_file)"
  elif [[ "$verify_fn" != "-" ]]; then
    if ! "$verify_fn" "$cname" "$log_file" >>"$log_file" 2>&1; then
      status="FAIL"; note="verify failed, see $log_file"
    fi
  fi
...
run_nonroot_no_sudo() {
  ...
  if [[ "$rc" != 1 ]]; then
    ...
    verify_nonroot_no_sudo "$cname" "$log" >>"$log" 2>&1 || { status="FAIL"; note="wrong/missing error message"; }
  fi
  record_result "$scen_id" "$status" "$note"
  cleanup_scenario "$cname" "$tag"
}
