...
One command does everything below: installs `uv` and the Claude CLI if missing, fetches the four
...
linger, then verifies all of it. Run it as your normal user, never root:
...
Nothing here is needed if the installer above finished. This path exists for developing on a clone,
where you want the repository copy running rather than the fetched one.
...
needed. `@reboot` in crontab won't do: `configuring_server.sh` from this repository restricts cron to
root only.
...
Two sources with the same lines: stdout goes to journald, and the same formatter writes `ping.log`
next to `main.py` — `~/.local/share/claude-auto-ping/` after the installer, the project directory
...
The file log is capped at 1 MB × 3 and survives reinstalling the unit. `.2` is the oldest part, `.1`
the middle one, the suffixless file is current:
...
Timestamps inside a line are MSK; journald prefixes its own in the host time zone, so on a non-MSK
server the two differ by the offset — expected, the schedule is computed in `Europe/Moscow` either
way.
